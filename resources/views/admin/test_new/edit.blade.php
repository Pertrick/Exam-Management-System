            ...
            <section class="content">
                <div class="container-fluid">
                    <div class="card card-info">
                        <!-- form start -->
                        <form action="{{ route('admin.test.new.update', $test->id) }}" method="POST" id="form-subject">
                            @csrf
                            @method('put')
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="card-header mb-3">
                                            <div style="display: flex; justify-content:space-between">
                                                <span class="fa fa-book"> Edit Exam -
                                                    {{ strtoupper($test->subject->name) }}</span>
                                                <a href="{{ route('admin.test.new.show', $test->id) }}"
                                        ...
                                        <div class="row" id="row-id">
                                                    ...
                                                    <select name="test_type" id="" class="form-control ">
                                                        ...
                                                        @foreach ($testTypes as $type)
                                                            <option value="{{ $type->id }}"
                                                                {{ $test->test_type_id == $type->id ? 'selected' : '' }}>
                                                                {{ $type->name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    ...
                                                    <input type="datetime-local" name="start_date" class="form-control"
                                                        value="{{ $test->start_date ? \Carbon\Carbon::parse($test->start_date) : '' }}" />
                                                    @error('start_date')
                                                        <div class="error text-danger text-xs">{{ $message }}</div>
                                                    @enderror
                                                </div>
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    ...
                                                    <input type="datetime-local" name="end_date"
                                                        value="{{ $test->end_date ? \Carbon\Carbon::parse($test->end_date) : '' }}"
                                                        class="form-control" />
                                                    @error('start_date')
                                                        <div class="error text-danger text-xs">{{ $message }}</div>
                                                    @enderror
                                            ...
                                            <div class="col-md-12">
                                                ...
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input" name="is_published" type="radio" id="inlineCheckbox1" value="1" {{ $test->is_published== 1 ? 'checked' : ''}}>
                                                    <label class="form-check-label" for="inlineCheckbox1">Open</label>
                                                  </div>
                                                  <div class="form-check form-check-inline">
                                                    <input class="form-check-input" name="is_published" type="radio" id="inlineCheckbox2" value="0"  {{ $test->is_published == 0 ? 'checked' : ''}}>
